...
navDay.getElementsByClassName("page-nav__day_chosen")[0].classList.toggle("page-nav__day_chosen");  // отключаем выбранный день по умолчанию 
navDay.children[0].classList.toggle("page-nav__day_chosen")  // делаем активным сегодняшний день

let pageNavDays = Array.from(document.getElementsByClassName("page-nav__day"));  // сменяем текущую дату
pageNavDays.forEach((e, index) => {
  e.onclick = function () {
    document.getElementsByClassName("page-nav__day_chosen")[0].classList.toggle("page-nav__day_chosen");  // отключаем текущий день
    e.classList.toggle("page-nav__day_chosen");  // назначаем выбранный 
...
function startSetInfo() {
  if (!localStorage.length) {  // обращаемся на сервер в случае отсутствия файлов
    request((info) => {
      localStorage.setItem('info', info);  // сохраняем информацию с сервера
      localStorage.setItem('date', new Date());  // записываем дату сохранения
    ...
    if (currentDate.getDate() === date.getDate()) {  // если дата сохранения сегодняшняя, то получаем информацию из локальных файлов
...
function setFilms(info, date) {  // функция принимает информацию которая приходит с сервера 1 запросом либо из localStroage, date - конкретный день и последующие дни, является Date
  let moviesSection = Array.from(document.getElementsByClassName("movie"));  // получаем все секции с фильмами
  let movieSection = moviesSection[0].cloneNode(true);  // создаем копию 1 секции
  ...
  Array.from(movieSection.getElementsByClassName("movie-seances__hall")).forEach(e => { e.remove() });  // удаляем информацию о сеансах
  let main = Array.from(document.getElementsByTagName("main"))[0];  // получаем элемент main в котором лежат секции с фильмами

  let filmsHalls = {};  // объект, ключ - id фильма, по ключу массив сеансов
  ...
  info.films.result.forEach((element) => {  // добавляем секции с новой информацией
    ...
    })  // после манипуляций в перменной curretnFilmHalls находится ввиде ключа название зала (зал1), а по ключу массив времени сеансов этого зала

    let sortedHalls = {};  // сортируем залы по порядку зал1, зал2, ...
    ...
    Object.keys(sortedHalls).forEach(key => {  // перебираем все залы и выводим информацию 
      ...
      sortedHalls[key].forEach(time => {  // перебираем время на конкретного фильма в конкретном зале
        ...
        refHall.setAttribute('data-price-vip', currHall.hall_price_vip);  // цена на vip-место
        ...
        refHall.onclick = function () {
          request((hallInfo) => {
            ...
            for (let i = 0; i < refHall.attributes.length; i++) {
              ...
              sessionStorage.setItem(attribute.name, attribute.value);  // сохраняем все атрибуты, которые находятся в теге а, что бы передать их на следующую страницу 
            }
            window.location.href = "hall.html";  // переходим на следующую страницу 
          }, data);
          return false;
        }
...
function declension(number, titles) {  // задаем правило правильного подбора окончаний ... минут, минута, минуты
