...
let sum = 0;  // стоимость отмеченных мест

if (hallInfo !== "null") {  // если сервер прислал нам разметку, то делаем следующее 
  ...
  let hallConfig = sessionStorage.getItem("hall_config");  // если с сервера пришёл "null", значит на этот сеанс, в этот зал билеты еще никто не купил и мы берем разметку из 1 запроса
  if (hallConfig) {
    wrapper.innerHTML = "";
...
let buyingInfoDescription = document.getElementsByClassName("buying__info-description")[0];  // меняем информацию о сеансе, берём данные которые сохраняли в sessionStroage при запросе на сервер
    ...
    chairs[j].onclick = function () {  // клик на место
      if (!this.classList.contains("conf-step__chair_taken")) {  // проверяем занято место или нет. Если не занято, то выбираем или отменяем выбор
        if (this.classList.toggle("conf-step__chair_selected")) {
          if (this.classList.contains("conf-step__chair_standart")) {  // добавляем стоимость за место
...
buyButton.onclick = function () {  // отправляем выбранное место 
  ...
  if (chairsSelected.length) {
    ...
    request(() => {
      ...
      window.location.href = "payment.html";  // переходим на следующую страницу 
    }, data);
  }
  else {
...
let buyingInfoHint = document.getElementsByClassName('buying__info-hint')[0];  // увеличение блока мест для маленьких экранов
let lastTouchEnd = 0;
let scale = "scale(1.0)";
let rem = "3rem";
let marginTop = "0";
